...
// Type: GameManager.InvadersFlyer
...
{
  public class InvadersFlyer : InvadersCritterBase
  {
    private float shootThreshold;
    private float shootTimer;

    public InvadersFlyer(GameScreen screen)
      : base(screen)
    {
    }
    ...
    {
      this.ship = new Sprite("flyer", Vector2.Zero, "sheets/invaders", this.Screen);
    ...
    public override void Update(Vector2 root, GameTime gameTime, ref Matrix worldTransform)
    {
      ...
      if ((double) this.shootTimer > (double) this.shootThreshold)
      {
        this.shootThreshold = Tools.RandomFloat(5f, 20f);
        this.shootTimer = 0.0f;
        ((InvadersScreen) this.Screen).fxNode.EnemyShootMissle(this.ship.Position.ToVector2());
      }
