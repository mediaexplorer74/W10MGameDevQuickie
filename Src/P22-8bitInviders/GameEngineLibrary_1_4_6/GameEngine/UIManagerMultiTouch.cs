...
// Type: GameEngine.UIManagerMultiTouch
...
{
  public class UIManagerMultiTouch : SceneNode, ITouchManager
  {
    public UIManagerMultiTouch.OnSliderValueChangedHandler FireSliderValueChanged;
    ...
    public event UIManagerMultiTouch.OnPressedWithNameHandler FirePressedWithName;

    public event UIManagerMultiTouch.OnTapBWithNameHandler FireTapWithName;

    public event UIManagerMultiTouch.OnTapHandler FireTap;

    public event UIManagerMultiTouch.OnPressedHandler FirePressed;

    public event UIManagerMultiTouch.OnReleasedHandler FireReleased;

    public event UIManagerMultiTouch.OnCheckedHandler FireChecked;
    ...
    public UIManagerMultiTouch(GameScreen screen)
    ...
    public bool HandleTouch(GestureSample gesture)
    {
      ...
      for (int touchIndex = 1; touchIndex <= 4; ++touchIndex)
      {
        for (int index = this.widgetListTemp.Count - 1; index >= 0; --index)
        {
          if (!this.skipID.Contains(this.widgetListTemp[index].Name) && this.widgetListTemp[index].HandleTouch(gesture, Engine.touchInputManager[touchIndex]))
          {
            this.widgetList.Remove(this.widgetListTemp[index]);
            this.widgetList.Add(this.widgetListTemp[index]);
            this.skipID.Add(this.widgetListTemp[index].Name);
          }
        }
      }
