...
// Type: GameManager.screen.WonMenu
...
namespace GameManager.screen
{
  public class WonMenu : Menu
  {
    ...
    public WonMenu(Game1 game, BinaryReader reader)
    {
    ...
    public override void Render(Screen screen)
    {
      ...
      string msg;
      if (num3 > 0)
        msg = num3.ToString() + "h" + (num4 < 10 ? (object) "0" : (object) string.Empty) + (object) num4 + "m";
      else
        msg = num4.ToString() + "m " + (num5 < 10 ? (object) "0" : (object) string.Empty) + (object) num5 + "s";
      ...
      Font.Draw(string.Empty + (object) this.game.ActivePlayer.Score, screen, 64, 48, Color.Get(-1, 550, 550, 550));
    }
