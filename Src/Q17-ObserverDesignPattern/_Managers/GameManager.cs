...
namespace GameManager
{
    ...
    public class GameManager
    {
        ...
        public GameManager()
        {
            _gate = Glob.Content.Load<Texture2D>("gate");
            _openGate = Glob.Content.Load<Texture2D>("opengate");
            Reset();
        }
        ...
        public void Reset()
        {
            ...
            _ui = new(Glob.Content.Load<SpriteFont>("font"), new(20, 20), _hero);
            _soundManager = new(_hero);

            var gem = Glob.Content.Load<Texture2D>("gem");
            for (int i = 1; i <= 10; i++) _gems.Add(
                new(gem, new(i * 64, 250)));
            for (int i = 1; i <= 5; i++) _gates.Add(
                new(_gate, _openGate, new(i * 100, 400), i * 2, _hero));
        }
