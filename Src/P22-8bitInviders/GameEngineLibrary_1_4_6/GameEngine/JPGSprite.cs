...
{
  public class JPGSprite : SceneNode
  {
    ...
    public JPGSprite(string sourcename, Vector2 position, string imageName, GameScreen screen)
    ...
    {
      this.tex = this.Screen.textureManager.LoadJPG(contentName);
    ...
    {
      this.tex = this.Screen.textureManager.LoadJPG(this.contentName);
      this.SourceRect = new Rectangle(0, 0, this.tex.Width, this.tex.Height);
      if (!(this.Pivot == Vector3.Zero))
        return;
      this.Pivot = new Vector3(this.Width / 2f, this.Height / 2f, 0.0f);
