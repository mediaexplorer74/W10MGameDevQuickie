...
namespace GameManager
{
    public class ProgressBarAnimated : ProgressBar
    {
        ...
        public ProgressBarAnimated(Texture2D bg, Texture2D fg, float max, Vector2 pos) 
            : base(bg, fg, max, pos)
        {
            _targetValue = max;
        ...
        public override void Update(float value)
        {
            ...
            {
                currentValue -= _animationSpeed * Glob.Time;
            ...
            _animationPart.Width = 
                (int)(Math.Abs(currentValue - _targetValue) / maxValue * foreground.Width);
        ...
        {
            base.Draw();
            Glob.SpriteBatch.Draw(foreground, _animationPosition, _animationPart, 
                _animationShade, 0, Vector2.Zero, 1f, SpriteEffects.None, 1f);
        }
    }
}
