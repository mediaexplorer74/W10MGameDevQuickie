...
namespace Win8.Core.Gestures
{
    ...
    public class GestureEventArgs : EventArgs
    {
        ...
        protected static Point GetPosition(UIElement relativeTo, Point point)
        {
            if (relativeTo == null)
            {
                ...
                relativeTo = default;//Application.Current.RootVisual;
            }
            if (relativeTo != null)
            {
                //RnD
    ...
    {
        internal DragDeltaGestureEventArgs(Point gestureOrigin, Point currentPosition, 
            Point change, Orientation direction) 
    ...
    public class PinchStartedGestureEventArgs : MultiTouchGestureEventArgs
    {
        ...
        public double Angle
        {
            get { return MathHelpers.GetAngle(TouchPosition2.X - TouchPosition.X, 
                TouchPosition2.Y - TouchPosition.Y); }
        }
    }
    ...
    {
        internal PinchGestureEventArgs(Point gestureOrigin, Point gestureOrigin2,
            Point position, Point position2)
        ...
        public double DistanceRatio
        {
            get
            {
                double originalDistance = Math.Max(MathHelpers.GetDistance(
                    GestureOrigin, GestureOrigin2), 1.0);

                double newDistance = Math.Max(MathHelpers.GetDistance(
                    TouchPosition, TouchPosition2), 1.0);
