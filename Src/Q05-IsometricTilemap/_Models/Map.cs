...
namespace GameManager
{
    ...
    public class Map
    {
        ...
        public Map()
        {
            ...
            Texture2D[] textures =
            {
            Glob.Content.Load<Texture2D>("tile0"),
            Glob.Content.Load<Texture2D>("tile1"),
            Glob.Content.Load<Texture2D>("tile2"),
            Glob.Content.Load<Texture2D>("tile3"),
            Glob.Content.Load<Texture2D>("tile4"),
        };
        ...
        {
            Vector2 cursor = new(mousePos.X - (int)(MAP_OFFSET.X * TILE_SIZE.X),
                mousePos.Y - (int)(MAP_OFFSET.Y * TILE_SIZE.Y));
        ...
        public void Update()
        {
            ...
            if (InputManager.Direction != Point.Zero)
            {
                ...
                //_keyboardSelected.X = Math.Clamp(_keyboardSelected.X + InputManager.Direction.X,
                //    0, MAP_SIZE.X - 1);

                if (_keyboardSelected.X + InputManager.Direction.X < 0)
                {
                    _keyboardSelected.X = 0;
                }
                else if (_keyboardSelected.X + InputManager.Direction.X > MAP_SIZE.X - 1)
                {
                    _keyboardSelected.X = MAP_SIZE.X - 1;
                }
                else
                {
                    _keyboardSelected.X = _keyboardSelected.X + InputManager.Direction.X;
                }

                if (_keyboardSelected.Y + InputManager.Direction.Y < 0)
                {
                    _keyboardSelected.Y = 0;
                }
                else if (_keyboardSelected.Y + InputManager.Direction.Y > MAP_SIZE.Y - 1)
                {
                    _keyboardSelected.Y = MAP_SIZE.Y - 1;
                }
                else
                {
                    _keyboardSelected.Y = _keyboardSelected.Y + InputManager.Direction.Y;
                }

                //_keyboardSelected.Y = Math.Clamp(_keyboardSelected.Y + InputManager.Direction.Y,
                //    0, MAP_SIZE.Y - 1);
