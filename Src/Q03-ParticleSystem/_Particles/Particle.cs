...
namespace GameManager
{
    ...
    public class Particle
    {
        ...
        public void Update()
        {
            _lifespanLeft -= Glob.TotalSeconds;
            if (_lifespanLeft <= 0f)
            {
                isFinished = true;
                return;
            ...
            _opacity = MathHelper.Clamp(MathHelper.Lerp(
                _data.opacityEnd, _data.opacityStart, _lifespanAmount), 0, 1);
            _scale = MathHelper.Lerp(_data.sizeEnd,
                _data.sizeStart, _lifespanAmount) / _data.texture.Width;
            _position += _direction * _data.speed * Glob.TotalSeconds;
        }

        public void Draw()
        {
            Glob.SpriteBatch.Draw(_data.texture, _position, null, 
                _color * _opacity, 0f, _origin, _scale, SpriteEffects.None, 1f);
        }
    }
}
