...
// Type: GameManager.IngameConfirmMusic
...
{
  public class IngameConfirmMusic : GameScreen
  {
    private UIManager uiNode;

    public IngameConfirmMusic()
    {
    ...
    public override void Initialize()
    {
      ...
      childItem.Add((SceneNode) new Sprite("bg", new Vector2(240f, 450f), "sheets/confirm", (GameScreen) this));
      ...
      this.uiNode.RegisterWidget((Widget) new WidgetRectangle("yes", new Rectangle(44, 517, 160, 60), (GameScreen) this)
      {
        ShowChrome = false,
        ShowWhenInteracted = false
      });
      this.uiNode.RegisterWidget((Widget) new WidgetRectangle("no", new Rectangle(294, 517, 160, 60), (GameScreen) this)
      {
        ShowChrome = false,
        ShowWhenInteracted = false
      });
    ...
    private void uiNode_FireTapWithName(int ID, string btnName)
    {
      ...
      switch (btnName)
      {
        case "yes":
          ...
          this.GetScreenByType<IngameOptionsScreen>().CheckMusic(true);
          this.ExitScreen();
          break;
        case "no":
          this.GetScreenByType<IngameOptionsScreen>().CheckMusic(false);
          this.ExitScreen();
          break;
      }
    }
